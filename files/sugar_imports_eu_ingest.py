"""
Hardmine — TDM Sugar EU Imports Ingest
========================================
Usage:
    python sugar_imports_eu_ingest.py            # incremental
    python sugar_imports_eu_ingest.py --full     # full history from 201401

Saves to: .../Sugar Flow/files/data/tdm_sugar_imports_eu.parquet
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOG_DIR / "sugar_imports_eu_ingest.log", encoding="utf-8"),
    ],
)
...
OUT_FILE = Path(__file__).parent / "data" / "tdm_sugar_imports_eu.parquet"
...
def fetch_tdm(period_begin: str) -> pd.DataFrame:
    ...
    log.info("Fetching Sugar EU Imports from %s ...", period_begin)
...
def main():
    parser = argparse.ArgumentParser(description="Sugar EU Imports Ingest")
    parser.add_argument("--full", action="store_true")
    args = parser.parse_args()

    log.info("=" * 60)
    log.info("Sugar EU Imports Ingest  |  %s", datetime.now().strftime("%Y-%m-%d %H:%M"))
