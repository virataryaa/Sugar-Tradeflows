"""
Hardmine — TDM Sugar Imports Ingest
=====================================
Usage:
    python sugar_imports_ingest.py            # incremental
    python sugar_imports_ingest.py --full     # full history from 201501

Saves to: .../Sugar Flow/files/data/tdm_sugar_imports.parquet
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOG_DIR / "sugar_imports_ingest.log", encoding="utf-8"),
    ],
)
...
REPORTERS = "ID,US,CN,MY,JP,SA,DZ,KR,IN,EG,MA,NG,PK"
...
OUT_FILE = Path(__file__).parent / "data" / "tdm_sugar_imports.parquet"
...
def fetch_tdm(period_begin: str) -> pd.DataFrame:
    ...
    log.info("Fetching Sugar Imports from %s ...", period_begin)
...
def main():
    parser = argparse.ArgumentParser(description="Sugar Imports Ingest")
    parser.add_argument("--full", action="store_true")
    args = parser.parse_args()

    log.info("=" * 60)
    log.info("Sugar Imports Ingest  |  %s", datetime.now().strftime("%Y-%m-%d %H:%M"))
